"""Cutting one observation into the patches it holds that carry enough measurement."""
...
def cut_patches(
    ...
    Args:
        observation: The observation to cut, whose axes say which of them are tiled.
        record: What the index says the observation is, which carries the ground it
            spans and when it was taken.
        patchsize: How far a patch of this instrument runs along every axis
            it is cut on.

    Yields:
        patch: Every whole patch of the observation, in the order its axes run, each
    ...
    """
    lengths = grid.patch_lengths(observation.values.shape, observation.axes, patchsize)
    counts = grid.patch_counts(observation.values.shape, observation.axes, patchsize)
    for one in range(math.prod(counts)):
        ...
        lon, lat = place.placement_of(observation, window)
        yield Patch(
            instrument=observation.instrument,
            identifier=observation.identifier,
            # Copied, so one patch does not hold the whole observation alive behind it.
